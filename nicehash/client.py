...
class NiceHashPrivateApi:
    ...
    # timestamp in milliseconds since 1.1.1970 (default value is now)
    def get_hashpower_earnings_for_currency(self, currency, timestamp=None, page=0, size=100):
        params = f'size={size}&page={page}'
        if timestamp:
            params += f'&timestamp={timestamp}'

        return self.request('GET', '/main/api/v2/accounting/hashpowerEarnings/' + currency, params, None)
    ...
    def get_withdrawals(self, currency, statuses=None, order_operation='LT', timestamp=None, page=0, size=100):
        params = {
            'size': size,
            'page': page,
            'op': order_operation
        ...
        return self.request('GET', f'/main/api/v2/accounting/withdrawals/{currency}', urlencode(params, True), None)
...
if __name__ == "__main__":
    ...
    private_api = NiceHashPrivateApi(options.base, options.org, options.key, options.secret)
